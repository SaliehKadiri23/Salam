...
const CommunitySection = ({ formData, onInputChange }) => {
  const interestOptions = [
    'Quran Studies', 'Hadith', 'Fiqh', 'Arabic Language', 'Islamic History',
    'Community Service', 'Youth Programs', 'Education', 'Charity Work',
    'Sports', 'Technology', 'Arts & Culture', 'Dawah', 'Interfaith Dialogue'
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {/* Interests */}
        <FormField label="Interests & Areas of Focus" helper="Select topics you're passionate about or want to learn more about">
          ...
          <div className="text-xs text-gray-500 mt-2 flex items-center gap-2">
            ...
            {(formData.interests || []).length >= 5 && (
              <span className="text-islamic-600 font-medium">Great variety! 🌟</span>
            )}
          </div>
        </FormField>

        {/* Community Role */}
        <FormField label="Community Role" helper="How do you contribute to the community?">
                ...
                >
                  <div className="flex items-center gap-3">
                    <Shield className={`w-5 h-5 transition-colors duration-200 ${
                      isSelected ? 'text-islamic-600' : 'text-gray-400 group-hover:text-islamic-500'
                    }`} />
                    <div>
                      <div className={`font-medium transition-colors duration-200 ${
                        isSelected ? 'text-islamic-700' : 'text-gray-900 group-hover:text-islamic-600'
                      }`}>
                        {role.label}
                      </div>
                    </div>
          ...
          {formData.communityRole && (
            <div className="mt-2 text-sm text-islamic-600 font-medium">
              ✓ Selected: {communityRoles.find(r => r.value === formData.communityRole)?.label}
            </div>
          )}
        </FormField>

        {/* Availability for Community Service */}
        <FormField label="Availability for Community Service" helper="How often can you contribute to community activities?">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              { value: 'weekly', label: 'Weekly', desc: 'Very active' },
              { value: 'monthly', label: 'Monthly', desc: 'Regular contributor' },
              { value: 'occasionally', label: 'Occasionally', desc: 'When possible' }
            ].map((option) => {

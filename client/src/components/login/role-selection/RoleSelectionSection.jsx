...
import { FaMosque, FaCrown, FaCheck } from 'react-icons/fa';
...
const RoleSelectionSection = ({ 
  ...
  return (
    <motion.section
      ...
      viewport={{ once: true }}
      className="bg-white/80 dark:bg-black/40 backdrop-blur-sm rounded-3xl p-8 pt-0 md:p-12 md:pt-0 shadow-2xl border border-white/20 dark:border-emerald-600 relative overflow-hidden"
    >
      {/* Background Pattern */}
      <div className="absolute top-0 right-0 size-32 opacity-5">
        <IslamicPattern
      ...
      <div className="text-center mb-5">
        ...
        />
        <h2 className="text-2xl md:text-3xl font-bold mt-[-20px] text-gray-800 dark:text-gray-100 mb-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {roles.map((role) => (
          <div
            ...
            className={`
              cursor-pointer p-6 rounded-2xl border-2 transition-all duration-300 hover:shadow-lg relative overflow-hidden hover:scale-105
              ${
                selectedRole === role.id
                  ? "border-green-500 bg-green-50 shadow-lg scale-105"
                  : "border-gray-200 dark:border-emerald-600 bg-white dark:bg-black/70 hover:border-green-300"
              }
            `}
          >
            {/* Selection indicator */}
      ...
      <AnimatePresence>
        {savedCredentials.hasRememberedCredentials && userData.role && (
          ...
          >
            <p className="text-islamic-700 dark:text-gray-100 text-sm mb-2">
      ...
      {/* Sign Up Link */}
      <div className="text-center pt-6 border-t border-gray-200 dark:border-emerald-600 mt-8">
        <p className="text-gray-600 dark:text-gray-100">
          New to Salam?{" "}
          <Link
            to={"/signup"}
            className="text-green-600 hover:text-green-700 dark:text-green-500 dark:hover:text-green-400 font-semibold transition-colors hover:underline"
          >
            Create an account
          </Link>
        </p>
